...
CFG = dict(open_melons=12,open_sheep=2,open_cows=2,open_wheat=7,open_hands=5,max_hands=12,
           ...
           crop_policy='legacy',crop_risk=.35,tomato_max=24,carrot_max=24)
_STATE = {}
...
def act_units_greedy(obs,st,work):
    ...
    for uid,pos in enumerate(positions):
        ...
        service_pending=CFG['finish_service'] and any(a[0] in ('FEED','CARE','WATER') for a in work.get(pos,([],0))[0])
        ...
        if cash_item and not service_pending and (central_sale or capital_due and cash_value>=(min(120,CFG['capital_value']) if early_fert else CFG['capital_value'])+back*CFG['delivery_distance_cost'] and back<=CFG['delivery_radius'] and hour+back+1<24):
            if back:
