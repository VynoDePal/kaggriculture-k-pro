"""K Pro optimization equivalence candidate: standalone deterministic public-observation policy.
Source SHA256: eb8e26ce3556d51f5841862dd5a5e0fe24c4763a27d5a06487ac3cec4f3677c1
Profile SHA256: 526e5d778fc7a561a00c41a93e8a233ca522e7a8782969b11ea7ebc1060a377b
...
def distance(a, b):
    return _DISTANCE[a[1] * 10 + a[0]][b[1] * 10 + b[0]]

def radius(p):
    return _RADIUS[p[1] * 10 + p[0]]
...
def crop_tasks(t, day, prices):
    ...
    if ready:
        acts.append(['HARVEST'])
    value = prices[c] * max(1, yu) + seed * 0.5
...
def act_units_greedy(obs, st, work, cache=None):
    me = obs['farms'][obs['player']]
    priv = obs['private']
    hour = obs['hour']
    day = obs['day']
    ...
    for uid, pos in enumerate(positions):
        ...
        for p, (todo, value) in work.items():
            ...
            for a in todo:
                ...
                sequence.append(a)
            d = distance(pos, p)
            if cache is not None and CFG['feed_supply_fix'] and (not any((a[0] == 'PLANT' for a in todo))):
                cache[uid, p] = (sequence, d)
            if not sequence:
...
def act_units(obs, st, work):
    ...
    cache = {}
    actions, shed = act_units_greedy(obs, st, work, cache)
