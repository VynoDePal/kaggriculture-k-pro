"""K Pro compiler smoke test: standalone deterministic public-observation policy.
Source SHA256: 1146a1ef1307f44528ecc37a9d2490f5f831440bac00dc7c738c30140b3b1b00
Profile SHA256: 32f701be4477b9318e35671062b6f1ab2fbda6af8f7df01d75ee718a553482ee
...
def plan(obs, st):
        ...
        if money > 400 and animal_n < limit:
            ...
            for a, data in ANIMAL.items():
                ...
                if a in ('COW', 'SHEEP'):
                    ...
                    milk_share = max(0.3, min(0.7, 0.5 + 0.012 * (demand(town, 'MILK') - demand(town, 'WOOL')) - 0.02 * (rival.get('COW', 0) - rival.get('SHEEP', 0))))
            ...
            if choices:
                value, a = max(choices)
                if value > 1.0:
                    targets[a] += min(3, limit - animal_n, species_room[a])
    pending = {a: priv['shed'].get(a, 0) + sum((i.get(a, 0) for i in priv['inventories'])) for a in ANIMAL}
    for a in ANIMAL:
        targets[a] = max(targets[a], ct.get(a, 0) + pending[a])
    herd = sum(targets.values())
    wheat_target = min(24, max(7, math.ceil(herd * 0.6))) if day <= 25 else 0
    ...
    if 11 <= day <= 18 and forecast(obs, 'MELON', 10) > 100:
        melon_target = max(melon_target, 6)
...
def animal_tasks(t, day, prices, town):
    ...
    if not t.get('cared_today') and (not skip) and (future_ticks > 0) and (prices[product] > prices['WHEAT'] * 0.5):
        acts.append(['CARE'])
...
def idle_deliveries(obs, actions, shed):
    ...
    hour = obs['hour']
    excess = sum(shed.values()) + sum((sum(i.values()) for i in invs)) - 90
    if excess <= 0:
        return (actions, shed)
    positions = [tuple(me['farmer'])] + [tuple(p) for p in me['hands']]
    choices = []
...
def market_orders(obs, st, shed):
    ...
    for item in items:
        ...
        if not amount:
            continue
        floor = max(1, int(PARAMS[item][0] * 0.4))
