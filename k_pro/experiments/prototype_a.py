...
CFG = dict(open_melons=12,open_sheep=2,open_cows=2,open_wheat=7,open_hands=5,max_hands=12,
           ...
           capital_delivery=True,capital_value=400,wheat_max=24)
_STATE = {}
...
def plan(obs,st):
    ...
    if day==0:
        ...
        limit=min(CFG['max_animals'],CFG['open_sheep']+max(0,day-3)*2)
    ...
    for a in ('SHEEP','COW','GOOSE'):
        for _ in range(max(0,targets[a]-ct.get(a,0))):
            if not free:break
            slot=min(free,key=lambda pos:(0 if kind(me['tiles'][pos[1]][pos[0]])==('COOP' if a=='GOOSE' else 'PASTURE') else 1,radius(pos),pos))
...
def tasks(obs,st):
    ...
    for pos,role in st['roles'].items():
        ...
        if k in CROP:acts,value=crop_tasks(t,day,prices)
            ...
            acts,value=([['PLANT',role],['WATER']],120) if obs['hour']<21 and day+CROP[role][1]<30 else ([],0)
...
def act_units(obs,st,work):
    ...
    if z is None or st.get('zone_count')!=n:z=zones(st,work,n)
    ...
    for uid,pos in enumerate(positions):
        ...
        if cash_item and capital_due and cash_value>=CFG['capital_value']+back*50 and back<=4:
            if back:
                actions.append(move(pos,nearest));continue
            amount=min(inv[cash_item],max(0,100-sum(shed.values())))
            if amount:
                actions.append(['PLACE',cash_item,amount]);shed[cash_item]=shed.get(cash_item,0)+amount;continue
...
def market_orders(obs,st,shed):
    ...
    for crop,data in CROP.items():
        needed=sum(role==crop and kind(me['tiles'][p[1]][p[0]]) in ('EMPTY','WEED') for p,role in st['roles'].items())-priv['seeds'].get(crop,0)
...
def agent(obs,configuration=None):
    ...
    st=_STATE[player]
    if st['day']!=obs['day'] or obs['hour'] in CFG['replan_hours'] and obs['farms'][player]['money']>600:
        plan(obs,st)
    work=tasks(obs,st)
