"""K Pro development laboratory; NOT the selected release artifact.
...
CFG defaults include rejected experimental settings. Run a frozen profile for
comparisons; the validated release will be published separately as k_pro1.py.
"""
import math
import copy
...
CFG = dict(open_melons=12,open_sheep=2,open_cows=2,open_wheat=7,open_hands=5,max_hands=12,
           ...
           critic_mode='legacy',tactic_set='logistics',lookahead_days=1,
           goose_demand_min=0,market_wheat=False,animal_labor_value=0.,stable_replans=False,
           hire_last_hour=8,lookahead_start_day=0,lookahead_replan_hours=(0,),
           central_policy='fixed',central_forecast_weight=.3,central_cow_bias=1.,
           central_harvest_first=False,central_harvest_value=200)
_STATE = {}
...
def plan(obs,st):
    ...
    if day==0:
        ...
        if money>400 and animal_n<limit:
            ...
            for a,data in ANIMAL.items():
                ...
                if a=='GOOSE' and demand(town,'EGG')<CFG['goose_demand_min']:continue
    ...
    if CFG['market_wheat'] and 4<=day<=CFG['late_wheat_day']:
        ratio=max(.7,min(1.7,(forecast(obs,'WHEAT',4)/PARAMS['WHEAT'][0])**.5))
    ...
    cow_central=CFG['milk_shed_slots']
    if CFG['central_policy']=='market':
        weight=CFG['central_forecast_weight']
        milk=((1-weight)*p['MILK']+weight*forecast(obs,'MILK',8))*1.5
        wool=((1-weight)*p['WOOL']+weight*forecast(obs,'WOOL',8))*4/3
        cow_central=milk*CFG['central_cow_bias']>=wool
...
def act_units_greedy(obs,st,work):
    ...
    for uid,pos in enumerate(positions):
        ...
        if CFG['central_harvest_first'] and back==0 and pos not in reserved and kind(standing) in ANIMAL:
            product=ANIMAL[kind(standing)][4]
            worth=standing.get('yield_units',0)*obs['market']['prices'][product]
            in_time=(hour+2<=deadline) if day==29 else hour<20
            if in_time and worth>=CFG['central_harvest_value']:
                actions.append(['HARVEST']);fixed.add(uid);reserved.add(pos);continue
...
def act_units(obs,st,work):
    ...
    fixed_tiles={pos[i] for i in fixed if actions[i][0]=='HARVEST' or actions[i][0]=='PLACE' and len(actions[i])>1 and actions[i][1] in ANIMAL}
...
def decide(obs,st):
    ...
    if st['day']!=obs['day'] or structural or obs['hour'] in CFG['replan_hours'] and obs['farms'][player]['money']>400:
        ...
        if keep_routes:st.update(previous)
            ...
            # An economic update need not move every service-sector boundary.
            # Preserve existing owners, attach new jobs to the closest sector.
            z=dict(previous['zones']);anchors=list(z)
            for pos in sorted(st['roles']):
                if pos not in z and anchors:z[pos]=z[min(anchors,key=lambda p:(distance(pos,p),p))]
...
def shadow_value(obs):
    ...
    if CFG['critic_mode']=='realizable':
        for row in farm['tiles']:
            for tile in row:
                k=kind(tile)
                if k in ANIMAL:item=ANIMAL[k][4]
                elif k in CROP and day-tile['planted_day']>=CROP[k][1]:item=k
                else:continue
                field_goods[item]=field_goods.get(item,0)+tile.get('yield_units',0)
        for item,n in field_goods.items():goods[item]=goods.get(item,0)+n
    ...
    for row in farm['tiles']:
        for tile in row:
            ...
            if k in ANIMAL:
                ...
                if CFG['critic_mode']=='realizable':quote=(prices[product]+forecast(obs,product,interval))/2
                ticks=max(0,(29-tile['placed_day']-first)//interval-(day-tile['placed_day']-first)//interval)
                if CFG['critic_mode']!='realizable':value+=tile.get('yield_units',0)*quote
                value+=min(cap-1,tile.get('pending_care_bonus',0))*quote*.85
                ...
                if CFG['critic_mode']=='realizable':quote=(prices[k]+forecast(obs,k,max(1,tile['planted_day']+first-day)))/2
                if CFG['critic_mode']!='realizable' or day-tile['planted_day']<first:value+=tile.get('yield_units',0)*quote
                if interval:
...
def choose_tactics(obs,st):
    ...
    if CFG['tactic_set']=='economic':
        choices=[{}, {'zone_penalty':3.}, {'zone_penalty':9.}, {'pickup_extra_food':1},
                 {'capital_delivery':False}, {'market_delivery':True,'market_delivery_value':400},
                 {'smart_fertilizer':True,'buy_fertilizer':True,'fertilizer_buy_ceiling':20}][:CFG['lookahead_choices']]
    elif CFG['tactic_set']=='terminal':
        choices=[{}]+[{'zone_penalty':v} for v in (0.,2.,4.,8.,10.,12.)]
...
def agent(obs,configuration=None):
    ...
    st=_STATE[player]
    if not CFG['lookahead'] or obs['day']<CFG['lookahead_start_day']:return decide(obs,st)
    if st.get('tactics_day')!=obs['day'] or obs['hour'] in CFG['lookahead_replan_hours'] and st.get('tactics_step')!=step:
        st['tactics']=choose_tactics(obs,st);st['tactics_day']=obs['day'];st['tactics_step']=step
    original=dict(CFG)
    try:
