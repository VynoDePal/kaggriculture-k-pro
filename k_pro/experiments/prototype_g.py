...
CFG = dict(open_melons=12,open_sheep=2,open_cows=2,open_wheat=7,open_hands=5,max_hands=12,
           ...
           react_land=True,straw_latest_day=16)
_STATE = {}
        ...
        if money>400 and animal_n<limit:
            ...
            for a,data in ANIMAL.items():
                ...
                if CFG['herd_policy']=='balanced' and a in ('COW','SHEEP'):
                    ...
                    if ct.get(a,0)+pending[a]>=math.ceil(CFG['max_animals']*share):continue
...
def act_units_greedy(obs,st,work):
    ...
    for uid,pos in enumerate(positions):
        ...
        service_pending=CFG['finish_service'] and any(
...
def agent(obs,configuration=None):
    ...
    if CFG['rotate_crops']:
        ...
        for pos,action in zip(positions,actions):
            ...
            if action[0]=='HARVEST' and kind(tile) in CROP and not CROP[kind(tile)][3]:st['rotation_pending']=True
    st['last_step']=step
