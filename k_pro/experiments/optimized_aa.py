...
def distance(a,b):return _DISTANCE[a[1]*10+a[0]][b[1]*10+b[0]]
def radius(p):return _RADIUS[p[1]*10+p[0]]
def move(p,q):
    ...
    for uid,pos in enumerate(positions):
        ...
        for p,(todo,value) in work.items():
            ...
            for a in todo:
                ...
                sequence.append(a)
            d=distance(pos,p)
            if cache is not None and CFG['feed_supply_fix'] and not any(a[0]=='PLANT' for a in todo):cache[(uid,p)]=(sequence,d)
            if not sequence:
...
def act_units(obs,st,work):
    ...
    for uid in workers:
        ...
        for p in tiles:
            ...
            if cached is not None:valid,d=cached
            else:
                for a in todo:
                    op=a[0]
