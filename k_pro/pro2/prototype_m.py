"""K Pro 2 laboratory M: transactional inventory and liquidation timing experiments.
Source SHA256: 56edca9c72d9e81859c078aebca4d44c588b8a5b7e2ca9fa6910643a38324c78
Profile SHA256: 8735202ba42e140bea94834acace27dabc4e51d87006cb91f4736d9b139c9789
...
def zones(st, work, n):
    ...
    if zone_mode == 'specialized' and n > 1:
        ...
        for group, offset, size in ((animal, 0, nh), (crops, nh, n - nh)):
            total = sum((len(work[p][0]) + 1.3 for p in group))
            done = 0.0
            for p in group:
...
def availability_signature(inventory, seeds):
    goods = sum(1 << i for i,item in enumerate(('WHEAT','FERTILIZER','COW','SHEEP','GOOSE')) if inventory.get(item,0))
    stock = sum(1 << i for i,item in enumerate(CROP) if seeds.get(item,0))
    return goods, stock


def feasible_actions(cache, pos, signature, todo, tile, role, inv, seeds, feed_fix):
    key = (pos, signature, feed_fix)
    ...
    for action in todo:
        ...
        if op == 'FEED' and inv.get('WHEAT',0) <= 0:
            requires = 'WHEAT'
            continue
        if feed_fix and op == 'CARE' and not inv.get('WHEAT') and not tile.get('fed_today'):
            continue
        if op in ('BUILD_PASTURE','BUILD_COOP') and inv.get(role,0) <= 0:
            requires = role
            break
        if op == 'PLACE' and inv.get(action[1],0) <= 0:
            requires = action[1]
            break
        if op == 'PLANT' and seeds.get(action[1],0) <= 0:
            break
        sequence.append(action)
    result = (tuple(sequence), requires)
    cache[key] = result
    return result


def act_units_greedy(obs, st, work, eligibility_cache=None):
    ...
    for uid, pos in enumerate(positions):
        ...
        if back == 0:
            ...
            if took:
                ...
                continue
        best = None
        signature = availability_signature(inv,seeds)
        for p, (todo, value) in work.items():
            ...
            if CFG['protect_targets'] and owner != uid and (distance(positions[owner], p) < distance(pos, p)):
                continue
            sequence, requires = feasible_actions(eligibility_cache,p,signature,todo,me['tiles'][p[1]][p[0]],st['roles'][p],inv,seeds,CFG['feed_supply_fix'])
            d = distance(pos, p)
...
def act_units(obs, st, work):
    ...
    eligibility_cache = {}
    actions, shed = act_units_greedy(obs, st, work, eligibility_cache)
    ...
    for uid in workers:
        ...
        for p in tiles:
            todo, value = work[p]
            valid, _ = feasible_actions(eligibility_cache,p,signature,todo,me['tiles'][p[1]][p[0]],st['roles'][p],inv,seeds,True)
            ...
            urgency = min(8, value / 250) * (1.8 if hour >= 17 else 1.0)
