"""K Pro 2 mpc-terminal-gate candidate: standalone deterministic public-observation policy.
Source SHA256: 736989b1109530088d7af1a5f582e7144f638290908a5398f91e2c95b4d4e4a8
Profile SHA256: 9c58583d308bad093134c00e36cacba70085101d0765ffa36713498bbe4ca251
...
CFG = {'zone_penalty': 6, 'zone_mode': 'angular', 'capital_delivery': True, 'pickup_extra_food': 0, 'smart_fertilizer': False, 'buy_fertilizer': False, 'fertilizer_buy_ceiling': 20, 'market_delivery': False, 'market_delivery_value': 250}
_STATE = {}
...
def terminal_features(obs):
    ...
    ct = counts(own)
    rt = counts(rival)
    available = sum((kind(t) in ('EMPTY', 'WEED') and radius((x, y)) <= 6 for y, row in enumerate(own['tiles']) for x, t in enumerate(row)))
    return {'wheat_quote': obs['market']['prices']['WHEAT'], 'carrot_quote': obs['market']['prices']['CARROT'], 'wheat_forecast': forecast(obs, 'WHEAT', 3), 'carrot_forecast': forecast(obs, 'CARROT', 3), 'available_tiles': available, 'own_wheat': ct.get('WHEAT', 0), 'own_strawberry': ct.get('STRAWBERRY', 0), 'own_animals': sum((ct.get(a, 0) for a in ANIMAL)), 'rival_wheat': rt.get('WHEAT', 0), 'rival_strawberry': rt.get('STRAWBERRY', 0), 'rival_animals': sum((rt.get(a, 0) for a in ANIMAL)), 'wheat_stock': obs['private']['shed'].get('WHEAT', 0) + sum((i.get('WHEAT', 0) for i in obs['private']['inventories']))}
...
def plan(obs, st):
    ...
    if st.get('terminal_active', False) and 22 <= day <= 27:
...
def crop_tasks(t, day, prices, rotation_active=False):
    ...
    if day < 29 or ready or (not interval and age >= first and (yu > 0)):
        ...
        if CFG['smart_fertilizer']:
            ...
            wants_fert = growing and t.get('fertilized_until_day', -1) < day and (c != 'WHEAT' or True)
...
def zones(st, work, n):
    zone_mode = 'specialized' if st['day'] >= 99 else CFG['zone_mode']
...
def act_units_greedy(obs, st, work):
    ...
    for uid, pos in enumerate(positions):
        ...
        service_pending = (CFG['market_delivery'] or day >= 99) and any((a[0] == 'WATER' and kind(standing) in CROP or (a[0] == 'FEED' and inv.get('WHEAT', 0) > 0) or (a[0] == 'CARE' and isinstance(standing, dict) and standing.get('fed_today')) for a in work.get(pos, ([], 0))[0]))
        central_sale = back == 0 and cash_value >= (min(70, 200) if early_fert else 200 if day >= 12 else 200) and (cash_item is not None)
        capital_due = CFG['capital_delivery'] and day <= 12 and (me['money'] < 4000) and (hour < 18)
        ...
        if CFG['market_delivery'] and cash_item and (hour < 21) and (back <= 2) and (cash_value / (back + 1) >= CFG['market_delivery_value']) and (obs['market']['prices'][cash_item] >= PARAMS[cash_item][0] * 0.5):
            deliver = True
...
def decide(obs, st):
    if obs['day'] >= 22 and 'terminal_active' not in st:
        features = terminal_features(obs)
        st['terminal_active'] = tree_decision(features, {'feature': 'wheat_forecast', 'threshold': 43.5, 'left': False, 'right': True})
    ...
    for row in farm['tiles']:
        for tile in row:
            ...
            if k in ANIMAL:
                ...
                value += 0.15 * (cost * 0.5 + ticks * (interval + 1) * quote)
            elif k in CROP:
                ...
                if day - tile['planted_day'] < first:
                    value += tile.get('yield_units', 0) * quote
                if interval:
                    ticks = sum((day < tile['planted_day'] + first + i * interval <= 29 for i in range(cap)))
                    value += 0.15 * ticks * 1.7 * quote
                elif tile['planted_day'] + first <= 29:
                    value += 0.15 * max(0, cap - tile.get('yield_units', 0)) * quote
    return value

def choose_tactics(obs, st):
    choices = [{}, {'zone_penalty': 3.0}, {'zone_penalty': 9.0}, {'pickup_extra_food': 1}, {'zone_mode': 'routing'}, {'zone_penalty': 0.0}, {'pickup_extra_food': 2}][:4]
    original = dict(CFG)
    results = []
    try:
        for patch in choices:
            ...
            remaining = min(24 * 2 - obs['hour'], 719 - obs['step'])
...
def agent(obs, configuration=None):
    ...
    if player not in _STATE or step <= _STATE[player].get('last_step', -1):
        _STATE[player] = {'day': -1}
    st = _STATE[player]
    if obs['day'] < 15 or False:
        return decide(obs, st)
    if st.get('tactics_day') != obs['day'] or (obs['hour'] in (0,) and st.get('tactics_step') != step):
