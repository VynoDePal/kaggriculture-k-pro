"""K Pro 2 computation-gate candidate: standalone deterministic public-observation policy.
Source SHA256: 736989b1109530088d7af1a5f582e7144f638290908a5398f91e2c95b4d4e4a8
Profile SHA256: 41580efaea8b0a6bb7258676064b052e16022812eeb82717c06ec00d269b4abe
...
def agent(obs, configuration=None):
    ...
    if player not in _STATE or step <= _STATE[player].get('last_step', -1):
        _STATE[player] = {'day': -1}
    st = _STATE[player]
    if obs['day'] >= 15 and 'use_lookahead' not in st:
        features = terminal_features(obs)
        st['use_lookahead'] = tree_decision(features, {'feature': 'available_tiles', 'threshold': 17.5, 'left': True, 'right': False})
        st['lookahead_gate_features'] = features
    if obs['day'] < 15 or not st.get('use_lookahead', False):
