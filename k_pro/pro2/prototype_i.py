"""K Pro 2 laboratory I: transactional inventory and liquidation timing experiments.
...
def animal_tasks(t, day, prices, town):
    ...
    if CFG['care'] and useful_care and (not t.get('cared_today')) and (not skip) and (future_ticks > 0) and (prices[product] > prices['WHEAT'] * 0.5):
        acts.append(['CARE'])
...
def tasks(obs, st):
    ...
    for pos, role in st['roles'].items():
        ...
        if k in CROP:
            ...
            acts, value = animal_tasks(t, day, prices, obs.get('town', {}))
        elif k == 'WEED':
            acts, value = ([['DIG']], 80)
...
def act_units_greedy(obs, st, work):
    ...
    for uid, pos in enumerate(positions):
        ...
        service_pending = (CFG['finish_service'] or CFG['market_delivery']) and any((a[0] == 'WATER' and kind(standing) in CROP or (a[0] == 'FEED' and inv.get('WHEAT', 0) > 0) or (a[0] == 'CARE' and isinstance(standing, dict) and standing.get('fed_today')) for a in work.get(pos, ([], 0))[0]))
