"""K Pro 2 laboratory J: transactional inventory and liquidation timing experiments.
...
def agent(obs, configuration=None):
    ...
    if player not in _STATE or step <= _STATE[player].get('last_step', -1):
        _STATE[player] = {'day': -1}
    st = _STATE[player]
    if not CFG['lookahead'] or obs['day'] < CFG['lookahead_start_day']:
