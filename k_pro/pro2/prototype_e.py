"""K Pro 2 laboratory E: transactional inventory and liquidation timing experiments.
...
def plan(obs, st):
    ...
    if CFG['terminal_rotation'] and CFG['rotation_start'] <= day <= CFG['rotation_last_plant']:
    ...
    if CFG['terminal_rotation'] and day >= CFG['rotation_start'] and not CFG['rotation_carrots']:
        carrot_target = 0
...
def crop_tasks(t, day, prices):
    ...
    if c == 'WHEAT' and (CFG['terminal_rotation'] or CFG['terminal_allocate']) and day >= CFG['rotation_harvest_day']:
...
def tasks(obs, st):
    ...
    for pos, role in st['roles'].items():
        ...
        k = kind(t)
        if k in CROP:
            acts, value = crop_tasks(t, day, prices)
        elif k in ANIMAL:
