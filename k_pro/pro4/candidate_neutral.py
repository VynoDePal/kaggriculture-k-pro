...
import math
import copy
CFG = {'early_melon_delivery': False, 'idle_overflow_delivery': False, 'project_market': False, 'wheat_autonomy': False, 'melon_replant_roi': False, 'fourth_quadrant': False}
...
def idle_deliveries(obs, actions, shed):
    ...
    for _, back, uid, item, qty, nearest in sorted(choices):
        ...
        if not room:
            break
        qty = min(qty, room)
        actions[uid] = move(positions[uid], nearest) if back else ['PLACE', item, qty]
        if not back:
...
def decide(obs, st):
    ...
    if CFG['project_market']:
        ...
        actions, _ = idle_deliveries(projected, actions, projected['private']['shed'])
        projected = project_unit_phase(obs, actions)
