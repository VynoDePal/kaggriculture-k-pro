...
def market_orders(obs, st, shed, seed_obs=None):
    me = obs['farms'][obs['player']]
    priv = obs['private']
    day = obs['day']
    hour = obs['hour']
    ...
    if not CFG['land_first']:
        acquire_land()
    seed_tiles = seed_obs['farms'][obs['player']]['tiles'] if CFG['seed_only_market'] and seed_obs is not None else me['tiles']
    seed_inventory = seed_obs['private']['seeds'] if CFG['seed_only_market'] and seed_obs is not None else priv['seeds']
    for crop, data in CROP.items():
        if CFG['seed_horizon_fix'] and day + data[1] >= 30:
            continue
        needed = sum((role == crop and kind(seed_tiles[p[1]][p[0]]) in ('EMPTY', 'WEED') and (not CFG['strict_diamond'] or len(me['unlocked_quadrants']) < 3 or radius(p) <= CFG['diamond_radius']) for p, role in st['roles'].items())) - seed_inventory.get(crop, 0)
...
def decide(obs, st):
    ...
    if CFG['post_unit_market'] and CFG['post_unit_start'] <= obs['day'] <= CFG['post_unit_end']:
        ...
        seed_obs = project_unit_phase(obs, actions) if CFG['seed_only_market'] else None
        orders = market_orders(obs, st, shed, seed_obs=seed_obs)
    if CFG['rotate_crops']:
