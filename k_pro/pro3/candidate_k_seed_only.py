"""K Pro 3 seed-only K: standalone deterministic public-observation policy.
Source SHA256: 17632c71be47c0d60b12dd0eba38426d01947a71a8d244ec282026f9dea71df2
Profile SHA256: 6c68f9fb888585c1ba7c3a64b6cc0ceaf833219d2b730c73031bf9c3bd736a6b
...
def market_orders(obs, st, shed, seed_obs=None):
    ...
    seed_tiles = seed_obs['farms'][obs['player']]['tiles'] if seed_obs is not None else me['tiles']
    seed_inventory = seed_obs['private']['seeds'] if seed_obs is not None else priv['seeds']
    for crop, data in CROP.items():
        if day + data[1] >= 30:
            continue
        needed = sum((role == crop and kind(seed_tiles[p[1]][p[0]]) in ('EMPTY', 'WEED') and (len(me['unlocked_quadrants']) < 3 or radius(p) <= 6) for p, role in st['roles'].items())) - seed_inventory.get(crop, 0)
...
def decide(obs, st):
    ...
    seed_obs = project_unit_phase(obs, actions)
...
def project_unit_phase(obs, actions):
    ...
    result = dict(obs)
    result['private'] = {'shed': dict(obs['private']['shed']), 'seeds': dict(obs['private']['seeds']), 'inventories': [dict(i) for i in obs['private']['inventories']]}
