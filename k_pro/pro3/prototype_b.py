"""K Pro 3 laboratory B: worker-feasible marginal task values.
Parent A SHA256: 8d3ff4c225526d26d1c9e84387bb000db296fdb142ec2551f55e2b61964108f5
...
def worker_task_value(tile, requested, feasible, value, day, prices):
    """Adjust only the marginal component to this worker's feasible actions."""
    weight = CFG['task_value_weight']
    if not CFG['feasible_task_value'] or not weight or day < CFG['task_value_start'] or requested == feasible:
        return value
    full = incremental_task_value(tile, requested, day, prices)
    available = incremental_task_value(tile, feasible, day, prices)
    if full is None or available is None:
        return value
    return max(1.0, value + weight * (available - full))
...
def assignment_urgency(value, hour, day):
    ...
    return min(CFG['urgency_cap'], value / divisor) * (1.8 if hour >= 17 else 1.0)
...
def act_units(obs, st, work):
    ...
    for uid in workers:
        ...
        for p in tiles:
            ...
            worker_value = worker_task_value(me['tiles'][p[1]][p[0]], todo, valid, value, day, obs['market']['prices'])
            urgency = assignment_urgency(worker_value, hour, day)
