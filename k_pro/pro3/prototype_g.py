...
def plan(obs, st):
    ...
    if (CFG['terminal_rotation'] or st.get('terminal_active',False)) and day >= CFG['rotation_start'] and not CFG['rotation_carrots']:
        carrot_target = 0
    tomato_target = ct.get('TOMATO', 0)
    if CFG['tomato_spot_cap'] > 0 and day < 20 and 4 * p['TOMATO'] >= 5 * PARAMS['TOMATO'][0]:
        tomato_target = max(tomato_target, CFG['tomato_spot_cap'])
    wanted = {'WHEAT': wheat_target, 'STRAWBERRY': straw_target, 'MELON': melon_target, 'TOMATO': tomato_target, 'CARROT': carrot_target}
    ...
    if CFG['reserve_animal_ring'] and day <= 16:
        ...
        free = free[protected:]
    crop_order = ('MELON', 'WHEAT', 'TOMATO', 'STRAWBERRY', 'CARROT') if CFG['tomato_spot_cap'] > 0 else ('MELON', 'WHEAT', 'STRAWBERRY', 'TOMATO', 'CARROT')
    for c in crop_order:
