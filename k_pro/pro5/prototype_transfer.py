...
import math
import copy
CFG = {'early_melon_delivery': True, 'idle_overflow_delivery': True, 'project_market': False, 'wheat_autonomy': False, 'melon_replant_roi': False, 'fourth_quadrant': False, 'near_opening_melons': True, 'wheat_transfer_ledger': True}
...
def market_orders(obs, st, shed):
    ...
    if CFG.get('wheat_transfer_ledger', False):
        # Use one consistent snapshot for feeding needs and owned wheat.
        # PICKUP/PLACE transfer existing goods; they do not create shortages
        # or duplicate stock. Keep the historical feeding timetable unchanged.
        owned = priv['shed'].get('WHEAT', 0) + carry['WHEAT']
        sold = sum(o[2] for o in orders if o[:2] == ['SELL', 'WHEAT'])
        target = unfed + sum(st['herd'].values()) // 3 if final_feeds is None else final_feeds
        wheat_need = max(0, target - max(0, owned - sold))
