...
import math
import copy
CFG = {'early_melon_delivery': True, 'idle_overflow_delivery': True, 'project_market': False, 'wheat_autonomy': False, 'melon_replant_roi': False, 'fourth_quadrant': False, 'near_opening_melons': True, 'wheat_purchase_projection': True}
...
def market_orders(obs, st, shed, wheat_state=None):
    me = obs['farms'][obs['player']]
    priv = obs['private']
    day = obs['day']
    hour = obs['hour']
    ...
    if wheat_state is not None:
        # Units act before market orders. Reconcile only wheat buying; leave
        # the frozen policy's sales, hiring, land and seed decisions unchanged.
        farm = wheat_state['farms'][wheat_state['player']]
        pocket = wheat_state['private']
        remaining = sum(isinstance(t, dict) and bool(t.get('animal'))
                        and not t.get('fed_today') for row in farm['tiles'] for t in row)
        target = remaining + sum(st['herd'].values()) // 3
        if final_feeds is not None:
            target = sum(any(a[0] == 'FEED' for a in todo)
                         for todo, _ in tasks(wheat_state, st).values())
        available = pocket['shed'].get('WHEAT', 0) + sum(i.get('WHEAT', 0) for i in pocket['inventories'])
        sold = sum(o[2] for o in orders if o[:2] == ['SELL', 'WHEAT'])
        wheat_need = max(0, target - max(0, available - sold))
...
def decide(obs, st):
    ...
    if CFG['project_market']:
        ...
        wheat_state = project_unit_phase(obs, actions) if CFG.get('wheat_purchase_projection', False) else None
        orders = market_orders(obs, st, shed, wheat_state=wheat_state)
    st['last_step'] = step
