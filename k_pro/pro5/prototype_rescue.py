...
def idle_feed_rescue(obs, actions):
    """Only redirect idle wheat carriers to desired, reachable feeding tasks."""
    if obs['day'] >= 29 or obs['hour'] < 12 or not any(a == ['PASS'] for a in actions):
        return actions
    post = project_unit_phase(obs, actions)
    farm = post['farms'][post['player']]
    positions = [tuple(farm['farmer'])] + [tuple(p) for p in farm['hands']]
    pockets = post['private']['inventories']
    targets = {}
    for y, row in enumerate(farm['tiles']):
        for x, tile in enumerate(row):
            if isinstance(tile, dict) and tile.get('animal'):
                todo, _ = animal_tasks(tile, post['day'], post['market']['prices'], post['town'])
                if any(a[0] == 'FEED' for a in todo):
                    targets[x, y] = tile.get('consecutive_unfed', 0)
    result = [list(a) for a in actions]
    for uid, action in enumerate(actions):
        if action != ['PASS'] or uid >= len(pockets) or pockets[uid].get('WHEAT', 0) <= 0:
            continue
        pos = positions[uid]
        reachable = [p for p in targets if obs['hour'] + distance(pos, p) + 1 <= 24]
        if not reachable:
            continue
        target = min(reachable, key=lambda p: (-targets[p], distance(pos, p), p))
        result[uid] = ['FEED'] if pos == target else move(pos, target)
        del targets[target]
    return result
...
def decide(obs, st):
    ...
    if CFG['project_market']:
        ...
        actions = idle_feed_rescue(obs, actions)
