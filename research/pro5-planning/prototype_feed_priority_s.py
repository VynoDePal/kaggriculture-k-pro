"""K Pro correction laboratory. Experimental, NOT a promoted submission.
Based on immutable K Pro 3 (55e4655b46896f1f4c0817eda9b38204b17bb8647d634034aadcdc48d65df97d).
Independent switches permit paired ablation; public observations only.
"""
import math
import copy
...
def plan(obs, st):
    ...
    if CFG['wheat_autonomy']:
        stock = priv['shed'].get('WHEAT', 0) + sum(i.get('WHEAT', 0) for i in priv['inventories'])
        wheat_target = wheat_acreage_target(day, herd, wheat_stock=stock)
    ...
    if CFG['melon_replant_roi'] and day > 0:
        workload = 4.8 * animal_n + 1.7 * sum(ct.get(c, 0) for c in CROP)
        spare = max(0, (len(me['hands']) + 1) * 20 - workload) * 10
        new_melons = late_melon_plant_count(day, forecast(obs, 'MELON', 10),
            money, 500 + 2 * herd * p['WHEAT'], available_actions=spare,
            maximum=max(0, 4 - ct.get('MELON', 0)))
        melon_target = ct.get('MELON', 0) + new_melons
    ...
    if day < 20 and 4 * p['TOMATO'] >= 5 * PARAMS['TOMATO'][0]:
        tomato_target = max(tomato_target, 2)
    ...
    if day <= 16:
        ...
        free = free[protected:]
    crop_order = ('MELON', 'WHEAT', 'TOMATO', 'STRAWBERRY', 'CARROT')
    for c in crop_order:
        for _ in range(max(0, wanted.get(c, 0) - ct.get(c, 0))):
            ...
            if not allowed:
                break
            slot = min(allowed, key=lambda pos: (-radius(pos) if c == 'MELON' and day < 2 and not CFG['near_opening_melons'] else radius(pos), pos))
            roles[slot] = c
            free.remove(slot)
    ...
    if CFG['melon_replant_roi']:
        st['new_melon_positions'] = {pos for pos, role in roles.items()
            if role == 'MELON' and kind(me['tiles'][pos[1]][pos[0]]) != 'MELON'}
...
def tasks(obs, st):
    ...
    for pos, role in st['roles'].items():
        ...
        if k in CROP:
            ...
            if CFG['melon_replant_roi'] and role == 'MELON' and day > 0:
                # A harvested tile retains its old role until replanning. Do not
                # let that stale role create an unbudgeted second wave at J10.
                if not st.get('new_melon_positions') or pos not in st['new_melon_positions']:
                    continue
...
def act_units_greedy(obs, st, work):
    ...
    for uid, pos in enumerate(positions):
        ...
        # Sell the first wave before waiting for automatic night deposit. Never
        # start a route that cannot deposit this day; protect urgent local care.
        urgent_here = any(a[0] in ('WATER', 'FEED') for a in work.get(pos, ([], 0))[0])
        if (CFG['early_melon_delivery'] and day in (10, 11)
                and cash_item == 'MELON' and hour + back < 24
                and cash_value >= 200 + 50 * (back + 1) and not urgent_here):
            deliver = True
        ...
        for p, (todo, value) in work.items():
            ...
            d = distance(pos, p)
            if not sequence:
                # The worker picks up at `nearest`, not at an independently
                # chosen access tile adjacent to the target.
                outbound = distance(nearest, p) if requires == 'WHEAT' else radius(p)
                supply_finish = hour + back + 2 + outbound
                # Hours 22 and 23 permit PICKUP then FEED on a shed tile.
                # Keep other supply policies unchanged; do not start a route
                # whose final feeding would occur after the night boundary.
                supply_fits = supply_finish < deadline or (requires == 'WHEAT' and supply_finish == deadline)
                if requires and shed.get(requires, 0) > 0 and supply_fits:
...
def _job_command(state, uid, job, role, allow_travel=False):
    p, action, _, _ = job
    farm = state['farms'][state['player']]
    positions = [tuple(farm['farmer'])] + [tuple(x) for x in farm['hands']]
    inv = state['private']['inventories'][uid]
    tile = farm['tiles'][p[1]][p[0]]
    k = kind(tile)
    op = action[0]
    day, hour = state['day'], state['hour']
    d = distance(positions[uid], p)
    if op == '_ROUTE':
        # One destination column for all remote work on this tile. Atomic
        # columns are only available to workers already standing on it.
        if d == 0:
            return None
        for rank, todo in enumerate(action[1]):
            command = _job_command(state, uid, (p, todo, rank, job[3]), role, True)
            if command is not None:
                return command
        return None
    if d and not allow_travel:
        return None
    deadline = 23 if day == 29 else 24
    if hour + d + 1 + (radius(p) + 1 if day == 29 else 0) > deadline:
        return None
    if day == 29 and op != 'HARVEST':
        return None
    legal = False
    if op == 'HARVEST':
        legal = k in ANIMAL and tile.get('yield_units', 0) > 0
        if k in CROP:
            legal = tile.get('yield_units', 0) > 0 and day - tile['planted_day'] >= CROP[k][1]
    elif op == 'FEED':
        legal = k in ANIMAL and not tile.get('fed_today') and inv.get('WHEAT', 0) > 0
    elif op == 'CARE':
        # Retain the existing economic guard, not a claimed engine restriction:
        # care is useful when fed already or this worker can supply wheat.
        legal = k in ANIMAL and (tile.get('fed_today') or inv.get('WHEAT', 0) > 0) and not tile.get('cared_today')
    elif op == 'COLLECT_FERTILIZER':
        legal = k in ANIMAL and tile.get('fertilizer_available')
    elif op == 'WATER':
        legal = k in CROP and not tile.get('watered_today')
    elif op == 'FERTILIZE':
        legal = k in CROP and tile.get('fertilized_until_day', -1) < day and inv.get('FERTILIZER', 0) > 0
    elif op == 'DIG':
        legal = k == 'WEED'
    elif op == 'PLANT':
        legal = tile is None and state['private']['seeds'].get(action[1], 0) > 0
    elif op in ('BUILD_PASTURE', 'BUILD_COOP'):
        legal = tile is None and inv.get(role, 0) > 0
    elif op == 'PLACE' and len(action) > 1 and action[1] in ANIMAL:
        structure = 'COOP' if action[1] == 'GOOSE' else 'PASTURE'
        legal = k == structure and inv.get(action[1], 0) > 0
    if not legal:
        return None
    return list(action) if d == 0 else move(positions[uid], p)


def assign_unit_tasks(obs, st, work, actions, fixed):
    """Joint atomic-job matching followed by canonical resource validation.

    Costs are inherited urgency heuristics, not an exact profit objective.
    Only already-held materials and seeds are promised. Earlier actions can
    unlock dependencies; later actions never lend resources to the prefix.
    """
    farm = obs['farms'][obs['player']]
    positions = [tuple(farm['farmer'])] + [tuple(p) for p in farm['hands']]
    jobs = []
    # Fixed operations already own their task, even if their unit acts later.
    seen = {(positions[uid], tuple(actions[uid])) for uid in fixed}
    for p in sorted(work):
        todo, value = work[p]
        remote = []
        for rank, action in enumerate(todo):
            key = (p, tuple(action))
            if key not in seen:
                seen.add(key)
                jobs.append((p, tuple(action), rank, value))
                remote.append(tuple(action))
        if remote:
            jobs.append((p, ('_ROUTE', tuple(remote)), -1, value))
    n = len(actions)
    workers = [uid for uid in range(n) if uid not in fixed]
    previous = dict(st.get('task_targets', {}))

    def cost(uid, job):
        p, action, rank, value = job
        tile = farm['tiles'][p[1]][p[0]]
        if (isinstance(tile, dict) and tile.get('animal')
                and tile.get('consecutive_unfed', 0) and not tile.get('fed_today')
                and any(a[0] == 'FEED' for a in work[p][0])
                and action[0] not in ('FEED', '_ROUTE')):
            # The inherited 1000-point survival urgency belongs to feeding,
            # not to collecting goods while the same animal escapes.
            value = max(0, value - 1000)
        result = distance(positions[uid], p) + 0.15 * (rank + 1)
        result -= assignment_urgency(value, obs['hour'], obs['day'])
        result += 6 * (st['zones'].get(p) != uid)
        if positions[uid] == p:
            result -= 2
        if previous.get(uid) == (p, action):
            result -= 0.4
        return result

    costs = []
    for uid in workers:
        row = [cost(uid, job) if _job_command(obs, uid, job, st['roles'].get(job[0])) is not None
               else 1000000.0 for job in jobs]
        costs.append(row + [50.0] * len(workers))
    selected = dict(zip(workers, minimum_assignment(costs)))
    committed = [['PASS'] for _ in actions]
    used = set()
    st['task_targets'] = {}
    for uid in range(n):
        if uid in fixed:
            committed[uid] = list(actions[uid])
            continue
        current = project_unit_phase(obs, committed[:uid]) if uid else obs
        later_plants = [actions[other][1] for other in fixed if other > uid
                        and actions[other][0] == 'PLANT']
        if later_plants:
            current = dict(current)
            current['private'] = dict(current['private'])
            current['private']['seeds'] = dict(current['private']['seeds'])
            for crop in later_plants:
                current['private']['seeds'][crop] = max(0, current['private']['seeds'].get(crop, 0) - 1)
        preferred = selected.get(uid, len(jobs))
        choices = []
        if preferred < len(jobs) and preferred not in used:
            command = _job_command(current, uid, jobs[preferred], st['roles'].get(jobs[preferred][0]))
            if command is not None:
                choices.append((cost(uid, jobs[preferred]), preferred, command))
        if not choices:
            reserved_later = {j for other, j in selected.items() if other > uid and j < len(jobs)}
            for j, job in enumerate(jobs):
                if j in used or j in reserved_later:
                    continue
                command = _job_command(current, uid, job, st['roles'].get(job[0]))
                if command is not None:
                    choices.append((cost(uid, job), j, command))
        if not choices:
            st['targets'].pop(uid, None)
            continue
        _, j, command = min(choices)
        committed[uid] = command
        used.add(j)
        st['targets'][uid] = jobs[j][0]
        st['task_targets'][uid] = (jobs[j][0], jobs[j][1])
    return committed


def act_units(obs, st, work):
    actions, shed = act_units_greedy(obs, st, work)
    return assign_unit_tasks(obs, st, work, actions, st['_fixed_units']), shed

def idle_deliveries(obs, actions, shed, original_obs=None):
    ...
    """
    if not CFG['idle_overflow_delivery'] or obs['hour'] < 14:
        return (actions, shed)
    # Caller supplies post-unit state when projection is enabled. Only PASS
    # commands are replaced: no feed, watering, or harvest task is stolen.
    me = obs['farms'][obs['player']]
    positions = [tuple(me['farmer'])] + [tuple(p) for p in me['hands']]
    inventories = obs['private']['inventories']
    excess = sum(shed.values()) + sum(sum(i.values()) for i in inventories) - 90
    if excess <= 0:
        return (actions, shed)
    actions, shed = list(actions), dict(shed)
    original_obs = obs if original_obs is None else original_obs
    herd = sum(counts(me).get(a, 0) for a in ANIMAL)
    food = sum(i.get('WHEAT', 0) for i in inventories) + shed.get('WHEAT', 0)
    choices = []
    for uid, (pos, inv, action) in enumerate(zip(positions, inventories, actions)):
        if action != ['PASS']:
            continue
        nearest = min(SHED, key=lambda p: (distance(pos, p), p))
        back = distance(pos, nearest)
        if obs['hour'] + back >= 24:
            continue
        for item in sorted(PARAMS):
            qty = inv.get(item, 0)
            if item == 'WHEAT':
                qty = min(qty, max(0, food - 2 * herd))
            if item == 'FERTILIZER':
                qty = max(0, qty - 5)
            if qty:
                value = qty * obs['market']['prices'][item] / (back + 1)
                choices.append((-value, back, uid, item, qty, nearest))
    used = set()
    for _, back, uid, item, qty, nearest in sorted(choices):
        if excess <= 0:
            break
        if uid in used:
            continue
        ...
        if qty <= 0:
            continue
        proposed = move(positions[uid], nearest) if back else ['PLACE', item, qty]
        if not back:
            # Inserting an earlier deposit must not steal room from an already
            # assigned later DROP, nor borrow room freed only by a later PICKUP.
            before = project_unit_phase(original_obs, actions)['private']
            trial = list(actions)
            trial[uid] = proposed
            after = project_unit_phase(original_obs, trial)['private']
            products = set(before['shed']) | set(after['shed'])
            for inv in before['inventories'] + after['inventories']:
                products.update(inv)
            if any(after['shed'].get(p, 0) + sum(i.get(p, 0) for i in after['inventories'])
                   < before['shed'].get(p, 0) + sum(i.get(p, 0) for i in before['inventories'])
                   for p in products):
                continue
            moved = before['inventories'][uid].get(item, 0) - after['inventories'][uid].get(item, 0)
            if moved <= 0:
                continue
            qty = moved
            shed = dict(after['shed'])
        actions[uid] = proposed
        used.add(uid)
        excess -= qty
        if item == 'WHEAT':
            food -= qty
...
def market_orders(obs, st, shed):
    ...
    def acquire_land():
        eligible = q < 3
        if CFG['fourth_quadrant'] and q == 3:
            diamond = [kind(t) for y, row in enumerate(me['tiles']) for x, t in enumerate(row)
                       if radius((x, y)) <= 6 and (x,y) not in SHED and kind(t) != 'LOCKED']
            occupied = sum(k in CROP or k in ANIMAL for k in diamond)
            eligible = fourth_quadrant_eligible(day, q, money, reserve, land_cost, occupied, len(diamond))
        if q < 4 and eligible and (land_day <= day <= 16) and (money > land_cost + reserve + 100):
    ...
    for crop, data in CROP.items():
        ...
        if CFG['melon_replant_roi'] and crop == 'MELON' and day > 0:
            needed = sum(kind(me['tiles'][p[1]][p[0]]) in ('EMPTY', 'WEED')
                         for p in st.get('new_melon_positions', ())) - priv['seeds'].get(crop, 0)
...
def decide(obs, st):
    ...
    if CFG['project_market']:
        projected = project_unit_phase(obs, actions)
        actions, _ = idle_deliveries(projected, actions, projected['private']['shed'], original_obs=obs)
        projected = project_unit_phase(obs, actions)
...
# Kaggle selects the last callable; preserve the tested policy unchanged.
