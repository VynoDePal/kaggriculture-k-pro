...
def plan(obs, st):
    ...
    for owned_first in (True, False):
        for a in ('SHEEP', 'COW', 'GOOSE'):
            amount = pending[a] if owned_first else max(0, targets[a] - ct.get(a, 0) - pending[a])
    ...
    for uid, pos in enumerate(positions):
        ...
        for p, (todo, value) in work.items():
            ...
            if not sequence:
                ...
                finish = hour + back + 2 + outbound
                fits = finish < deadline or (requires == 'WHEAT' and finish == deadline)
                if requires == 'WHEAT' and fits and finish == 24 and radius(p) == 0 and day < 28:
                    # Experimental deferral: no immediate gain, accessible tomorrow.
                    # Central placement is not a guarantee of tomorrow's service.
                    tile = me['tiles'][p[1]][p[0]]
                    without = animal_after_night(tile, day)
                    with_feed = animal_after_night(tile, day, feed=True)
                    if ('animal' in without
                            and without.get('yield_units', 0) == with_feed.get('yield_units', 0)
                            and without.get('pending_care_bonus', 0) == with_feed.get('pending_care_bonus', 0)):
                        continue
                if requires and shed.get(requires, 0) > 0 and fits:
