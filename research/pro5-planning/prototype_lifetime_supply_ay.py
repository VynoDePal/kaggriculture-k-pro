...
def forecast(obs, item, horizon=5):
    ...
    horizon = max(0, min(horizon, 29 - obs['day']))
        ...
        for row in farm['tiles']:
            for t in row:
                ...
                if t.get('crop') == item:
                    ...
                    if cd[3]:
                        ticks = sum(obs['day'] < t['planted_day'] + cd[1] + i * cd[3] <= obs['day'] + horizon
                                    for i in range(cd[4]))
                        # Assumes harvest/sale between events and two serviced units per event.
                        # Plant holding capacity does not cap cumulative marketed production.
                        supply += t.get('yield_units', 0) + 2 * ticks
                    elif age + horizon >= cd[1]:
                        supply += min(cd[4], t.get('yield_units', 0) + horizon / 4 * 2)
...
def plan(obs, st):
    ...
    if day >= 3 and forecast(obs, 'STRAWBERRY', CROP['STRAWBERRY'][1]) < 20:
