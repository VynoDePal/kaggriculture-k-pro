...
def last_chance_return(obs, st, work, actions, shed):
    """At most one harvest-bound mover returns before its delivery deadline."""
    if obs['day'] >= 29 or obs['hour'] < 14:
        return actions
    inventories = obs['private']['inventories']
    if sum(shed.values()) + sum(sum(inv.values()) for inv in inventories) <= 90:
        return actions
    me = obs['farms'][obs['player']]
    positions = [tuple(me['farmer'])] + [tuple(p) for p in me['hands']]
    choices = []
    for uid, (pos, inv, action) in enumerate(zip(positions, inventories, actions)):
        if action[0] not in ('NORTH', 'SOUTH', 'EAST', 'WEST'):
            continue
        target = st['targets'].get(uid)
        todo, value = work.get(target, ([], 0))
        if not todo or any(a[0] != 'HARVEST' for a in todo):
            continue
        if any(a[0] in ('WATER', 'FEED') for a in work.get(pos, ([], 0))[0]):
            continue
        nearest = min(SHED, key=lambda p: (distance(pos, p), p))
        back = distance(pos, nearest)
        if not back or obs['hour'] + back != 23:
            continue
        cargo_value = max((n * obs['market']['prices'][item] for item, n in inv.items()
                           if item in PARAMS and item not in ('WHEAT', 'FERTILIZER')), default=0)
        if cargo_value > value:
            choices.append((-cargo_value, uid, nearest))
    if choices:
        _, uid, nearest = min(choices)
...
def decide(obs, st):
    ...
    actions = last_chance_return(obs, st, work, actions, shed)
